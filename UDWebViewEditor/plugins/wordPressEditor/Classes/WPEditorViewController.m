...
@interface WPEditorViewController () <HRColorPickerViewControllerDelegate, BKEditorFormatbarViewDelegate, WPEditorViewDelegate>
...
- (void)editorToolbarView:(BKEditorFormatbarView *)editorToolbarView atUser:(UIBarButtonItem *)barButtonItem{
    [self insertName:@"@测试插入用户名" url:@"--"];
}

- (void)editorToolbarView:(BKEditorFormatbarView *)editorToolbarView addTag:(UIBarButtonItem *)barButtonItem{
    NSLog(@"标题内容----%@", self.titleText);
    NSLog(@"正文内容----%@", self.bodyText);
    
    NSString *lJs = @"document.documentElement.innerHTML";
    
    NSString *html = [self.editorView.webView stringByEvaluatingJavaScriptFromString:lJs];
    
    NSLog(@"%@", html);
}


//- (void)editorToolbarView:(BKEditorFormatbarView *)editorToolbarView
...
- (void)insertName:(NSString*)name
              url:(NSString*)url{
    [self.editorView insertUserName:name url:url];
}

- (void)insertEmoji:(NSString*)emoji{
    [self.editorView insertEmoji:emoji];
}
