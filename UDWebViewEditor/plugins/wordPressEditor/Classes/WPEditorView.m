...
@property (nonatomic, strong, readwrite) NSString *selectedUserTitle;
...
//        [self createSourceTitleViewWithFrame: childFrame];
        [self createSourceDividerViewWithFrame:CGRectMake(0.0f,
                                                          CGRectGetMaxY(self.sourceViewTitleField.frame),
...
- (void)processStyles:(NSString *)styles
{
    ...
    for (NSString *styleString in styleStrings) {
        ...
        } else if ([styleString hasPrefix:@"user-title:"]) {
            self.selectedUserTitle = [self stringByDecodingURLFormat:[styleString stringByReplacingOccurrencesOfString:@"user-title:" withString:@""]];
...
- (void)insertUserName:(NSString *)name url:(NSString *)url{
    
    NSString *trigger = [NSString stringWithFormat:@"ZSSEditor.atUser('%@', '%@')", name, url];
    [self.webView stringByEvaluatingJavaScriptFromString:trigger];
    [self.webView stringByEvaluatingJavaScriptFromString:@"ZSSEditor.insertHTML(&nbsp;)"];
...
- (void)insertEmoji:(NSString*)emoji{
    ...
    unsigned long  unicodeIntValue= strtoul([emoji UTF8String],0,16);
    ...
    NSString *trigger = [NSString stringWithFormat:@"ZSSEditor.insertEmoji('%@')", sendStr];
