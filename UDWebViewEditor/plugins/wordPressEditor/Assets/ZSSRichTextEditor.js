...
//        ZSSEditor.currentEditingUser = null;
...
ZSSEditor.refreshVisibleViewportSize = function() {
    ...
    console.log("content-top", $('#zss_field_content').position().top);
...
//to do: <a></a> need to be replaced by <bk-user></bk-user>
ZSSEditor.atUser = function(text,url){
    var html = '<bk-user>'+text+'</bk-user>&zwnj;';
    var focusedField;
    try{
...
ZSSEditor.insertEmoji = function(emoji){
...
ZSSEditor.insertName = function(name){
    var html = '<bk-user>' + name + '</bk-user>';
    this.insertHTML(html);
    this.sendEnabledStyles();
}
...
//    console.log('test1', document.getElementById('test'));
//    console.log('body1', document.getElementById('test').innerHTML);
    
	document.execCommand('insertHTML', false, html);//$('<div/>').text(html).html()
//    console.log('test2', document.getElementById('test'));
//    console.log('body2', document.getElementById('test').innerHTML);
    console.log('test3', $("#zss_field_content"));
	this.sendEnabledStyles();
};
...
ZSSEditor.sendEnabledStyles = function(e) {
    ...
    if (focusedField && !focusedField.hasNoStyle) {
        ...
        for (var i = 0; i < parentTags.length; i++) {
            ...
            } else if (currentNode.NodeName.toLowerCase() == 'span'){
                ZSSEditor.currentEditingUser = currentNode;
                var title = encodeURIComponent(currentNode.text);
                
                items.push('span');
            }
//            else if(currentNode.nodeName.toLowerCase() == 'bk-user'){
//                    ZSSEditor.currentEditingUser = currentNode;
//                    var title = encodeURIComponent(currentNode.innerHTML);
//                    items.push('user-title:' + title);
//                }
        }
